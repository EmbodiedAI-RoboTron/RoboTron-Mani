...
class BaseCalvinDataset(Dataset):
    ...
    def __init__(
        ...
        with open('/project/RoboFlamingo/enrich_lang_annotations.json', 'r') as f:
    ...
    def process_rgb(
        ...
        for _, rgb_obs_key in enumerate(rgb_obs_keys):
            ...
            else:  # episode loader # yiyang-modify 改输入图片数量应该改这里
    ...
    def __getitem__(self, idx: Union[int, Tuple[int, int]], fixed_seed=False) -> Dict:
        ...
        for i in range(np_gripper.shape[0]):
            new_list.append(Image.fromarray(np_gripper[i, :, :, :].astype(np.uint8))) #uint8格式转化
...
class DiskCalvinDataset(BaseCalvinDataset):
        ...
        **kwargs: Any,
        ...
        if self.with_lang:
            (
                ...
                self.lang_ann,
                self.lang_task
            ) = self._build_file_indices_lang(self.abs_datasets_dir) # 从实际路径构建，training文件夹
        else:
            self.episode_lookup = self._build_file_indices(self.abs_datasets_dir)
            # episode_lookup epis ode中每个end_idx - window_size ～ end_idx

        # self.naming_pattern, self.n_digits = lookup_naming_pattern(
        #     self.abs_datasets_dir, self.save_format
        # ) /data/robotics/calvin/training/dict.json

        # yiyang_modify
        self.json_file_path = Path("/data/robotics/calvin/training/dict.json")
        ...
        self.ColorJitter = transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5)
    ...
    def _get_episode_name_split(self, file_idx: int) -> Path:
        ...
        return Path(self.file_path[f"{os.path.basename(self.naming_pattern[0])}{file_idx:0{self.n_digits}d}{self.naming_pattern[1]}"])
        
        # num_folder = str(self.file_dict[str(file_idx)])
        # return Path(
        #     f"{self.abs_datasets_dir}/{num_folder}/episode_{file_idx:0{7}d}.npz"
        # )
    ...
    def _load_episode(self, idx: int, window_size: int) -> Dict[str, np.ndarray]:
        ...
        keys.append('actions')
        episodes = [
            self.load_file(self._get_episode_name_split(file_idx))
    ...
    def _build_file_indices_lang(
        ...
        lang_task = lang_data["language"]["task"] # 所有的任务指令字符串
        lang_lookup = []
        ...
        for i, (start_idx, end_idx) in enumerate(ep_start_end_ids):
            if self.partial_data:
                if (start_idx, end_idx) not in partial_st_ed_list:
                    continue
            if self.pretrain: # yiyang_question
                start_idx = max(
                    start_idx,
                    end_idx + 1 - self.min_window_size - self.aux_lang_loss_window,
                )
            # if lang_task[i] != "lift_red_block_table":
            #     continue
            assert end_idx >= self.max_window_size
            cnt = 0

            # window_size 对应的是lookup的index，episode表示轨迹中，lang_lookup表示的是第几个任务
            for idx in range(start_idx, end_idx + 1 - self.min_window_size):
                if cnt % self.skip_frames == 0: #yiyang_question
    ...
    def collater(self, sample):
        ...
        if self.use_aug :
            for i, s in enumerate(sample):
                for ii, img in enumerate(s["rgb_obs"]["rgb_static"]) :
                    sample[i]["rgb_obs"]["rgb_static"][ii] = self.ColorJitter(img)
                for ii, img in enumerate(s["rgb_obs"]["rgb_gripper"]) :
                    sample[i]["rgb_obs"]["rgb_gripper"][ii] = self.ColorJitter(img)


        image_tensors = torch.stack([self.image_fn(s["rgb_obs"]["rgb_static"]) for s in sample])
        #image_tensors = torch.zeros_like(image_tensors)
        gripper_tensors = torch.stack([self.image_fn(s["rgb_obs"]["rgb_gripper"]) for s in sample])
        #gripper_tensors = torch.zeros_like(gripper_tensors)
...
class CalvinSim(Dataset):
    def __init__(self,  
        ...
        # dataset_path = "/data/robotics/task_D_D"
        # calvin_conf_path = "/project/robotic/calvin/calvin_models/conf"
