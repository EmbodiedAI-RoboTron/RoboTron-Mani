...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, tensor_list):
        ...
        dim_t = torch.arange(self.num_pos_feats, dtype=dtype, device=tensor_list.device)
...
def build_position_encoding(args):
    ...
    if args.model.position_embedding in ('v2', 'sine'):
        # TODO find a better way of exposing other arguments
        position_embedding = PositionEmbeddingSine(N_steps, normalize=True)
    elif args.model.position_embedding in ('v3', 'learned'):
        position_embedding = PositionEmbeddingLearned(N_steps)
    else:
        raise ValueError(f"not supported {args.model.position_embedding}")
